...
//[1] scriptlet 태그:이 안에서는 자바코드를 기술
/*자바주석
[2] expression(출력식)*/
	Date today=new Date();
...
<%--3.JSP주석(권장)
Script Element
[1] scriptlet: <% %> : (스크립트랫)자바코드 => 여기서 선언된 변수는 지역변수
[2] expression: <%= %> : (출력식)변수나 메서드의 반환값을 브라우저에 출력하고자 할때 사용
[3] declaration: <%! %> : (선언문)변수 선언하거나 사용자 정의 메서드를 이 안에서 구성할 수 있다 => 여기서 선언된 변수는 맴버변수
--%>
<%!
	String global="인스턴스 변수(맴버변수)";
