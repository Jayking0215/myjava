...
public class Weka02DecisionTree {
	static String file1="C:\\Weka-3-9\\data\\UniversalBank\\UniversalBank_preprocess.arff";
	static String file2="C:\\Weka-3-9\\data\\UniversalBank\\UniversalBank_realData.arff";
	Instances data, train, test;
	//Classifier model;//J48...강제 형변환 해야함
	J48 model;
	...
	private void generateModel_Evaluate() {
		//모델 생성
		model=new J48();//DecisionTree모델
		
		model.setMinNumObj(10);//노드 당 최소 인스턴스 수 지정=>설정값이 클수록 노드수가 줄어들어 나무의 깊이도 낮아짐
		model.setUnpruned(false);//가지치기 하려면 false지정
		try {
			...
			int nums=(int)eval.numInstances();//분류대상 데이터 수
			int accuracy=(int)eval.correct();//정분류 건수
			int percent=accuracy*100/nums;//정분류율
			System.out.println("분류대상 데이터 수: "+nums);
	...
	private void treeViewInstances(Instances data2, J48 model, Evaluation eval) throws Exception {
		...
		f.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);//창닫기
		f.getContentPane().add(panel,"Center");
		f.getContentPane().add(sp,"South");
		
		f.setSize(800,800);
		f.setVisible(true);
		//panel.fitToScreen();//스크린에 맞게 채움
	}//------------------
	...
	private void testPredict(String file) {
		try {
			...
			for(int i=0; i<predData.numInstances(); i++) {
				...
				System.out.println("predict value: "+predData.classAttribute().value((int)pred));
