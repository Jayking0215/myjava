...
public class Weka03Logisticegression {
	
	String upDir="C:\\\\Weka-3-9\\\\data\\\\UniversalBank\\";
	...
	public void loadArff(String file) throws Exception{
		...
		data=ds.getDataSet();
		//로지스틱회귀는 데이터를 정규화해야 한다.==>weka에서는 filter로 제공됨
		Normalize normalize=new Normalize();
		//정규화 filter를 data에 적용시키자
		normalize.setInputFormat(data);
		
		Instances newData=Filter.useFilter(data, normalize);
		//newData가 정규화된 데이터=>학습 데이터와 테스트 데이터로 분리
	...
	public void generateModel_Evalueate() throws Exception{
		...
		//학습 모델을 저장
		saveModel(upDir+"UniversalBank.model");
	}
	public void testPredict(String file) throws Exception{
		//저장했던 학습 모델을 복원시키자
		Classifier model2=loadModel(upDir+"UniversalBank.model");
		...
		for(int i=0; i<newData.numInstances(); i++) {
			...
			System.out.println("예측한 데이터값: "+newData.classAttribute().value((int)pred));
	...
	public static void main(String[] args) throws Exception{
		String file1="C:\\Weka-3-9\\data\\UniversalBank\\UniversalBank_preprocess.arff";
		String file2="C:\\Weka-3-9\\data\\UniversalBank\\UniversalBank_realData.arff";
		
		Weka03Logisticegression app=new Weka03Logisticegression();
		app.loadArff(file1);
		//app.generateModel_Evalueate();
